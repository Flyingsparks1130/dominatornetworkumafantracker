(function attachDeeperInsightsPage() {
  ...
  const PANEL = { background: "rgba(29,23,28,0.92)", border: "1px solid #3b2932", borderRadius: 14, padding: "16px 18px" };
  const MUTED = { color: "#ae9aa5", fontSize: 11, lineHeight: 1.5 };
  ...
  function MetricCard({ label, value, sub, color = "#f3e9ee" }) {
    return (
      <div style={{ ...PANEL, padding: "13px 15px", minHeight: 92 }}>
        <div style={{ color: "#ae9aa5", fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 5 }}>{label}</div>
  ...
  function EmptyState({ title, detail }) {
    return <div style={{ ...PANEL, textAlign: "center", padding: "34px 20px" }}><div style={{ color: "#ddc9d3", fontSize: 14, fontWeight: 800, marginBottom: 5 }}>{title}</div><div style={MUTED}>{detail}</div></div>;
  }
  ...
  function ExplanationBox({ headline, meaning, action, color = "#60a5fa" }) {
    return <div style={{ background: `${color}0d`, border: `1px solid ${color}44`, borderRadius: 12, padding: "13px 15px", marginBottom: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(230px,1fr))", gap: 12 }}>
        <div><div style={{ color: "#c3a7b2", fontSize: 9, fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 3 }}>What this means</div><div style={{ color: "#ddc9d3", fontSize: 11, lineHeight: 1.55 }}>{meaning}</div></div>
        <div><div style={{ color: "#c3a7b2", fontSize: 9, fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 3 }}>What to do with it</div><div style={{ color: "#ddc9d3", fontSize: 11, lineHeight: 1.55 }}>{action}</div></div>
      </div>
    </div>;
  }
  ...
  function TargetAttainmentDotPlot({ clubs, snapshotsById, forecastsById, selectedClubId, setSelectedClubId, analysisDay }) {
    ...
    return <div style={{ ...PANEL, marginBottom: 14 }}>
      ...
      <div style={{ marginLeft: 126, marginRight: 58, height: 18, position: "relative" }} className="insights-dot-axis">{ticks.map((tick) => <span key={tick} style={{ position: "absolute", left: position(tick), transform: "translateX(-50%)", color: tick === 1 ? "#fbbf24" : "#a18c98", fontSize: 8, fontWeight: tick === 1 ? 900 : 600 }}>{Math.round(tick * 100)}%</span>)}</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{rows.map((row) => {
        ...
        return <button key={row.club.id} onClick={() => setSelectedClubId(String(row.club.id))} style={{ display: "grid", gridTemplateColumns: "118px minmax(230px,1fr) 52px", gap: 8, alignItems: "center", background: String(row.club.id) === String(selectedClubId) ? "rgba(169,63,86,0.10)" : "transparent", border: 0, borderRadius: 8, padding: "5px 4px", cursor: "pointer", textAlign: "left" }} className="insights-dot-row">
          ...
          <span style={{ height: 18, position: "relative", display: "block", background: "#151115", borderRadius: 999, border: "1px solid #3b2932" }}>
            ...
            <span title={`Earned through Day ${analysisDay}: ${pct(row.currentRatio)}`} style={{ position: "absolute", left: position(row.currentRatio), top: 4, width: 8, height: 8, transform: "translateX(-50%)", borderRadius: 999, border: "2px solid #9ca3af", background: "#151115", boxSizing: "border-box" }} />
      ...
      <div style={{ ...PANEL, overflow: "hidden", padding: 0 }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 1060 }}>
            <thead>
              <tr style={{ background: "rgba(21,16,21,0.82)" }}>
                {["Club", "Gain so far", "Full club goal", "Best month-end estimate", "Working range", "Goal status", "Estimated final rank", "Data confidence"].map((header) => <th key={header} style={{ color: "#ae9aa5", textAlign: "left", fontSize: 9, textTransform: "uppercase", letterSpacing: "0.08em", padding: "11px 12px", borderBottom: "1px solid #3b2932", whiteSpace: "nowrap" }}>{header}</th>)}
              </tr>
            </thead>
            <tbody>
              {clubs.map((club) => {
                ...
                return (
                  <tr key={club.id} onClick={() => snapshot && setSelectedClubId(String(club.id))} style={{ cursor: snapshot ? "pointer" : "default", background: selected ? "rgba(169,63,86,0.12)" : "transparent", borderBottom: "1px solid #261c23" }}>
                    <td style={{ padding: "11px 12px" }}><div style={{ color: "#f3e9ee", fontWeight: 800, fontSize: 12 }}>{club.name}</div><div style={{ color: "#ae9aa5", fontSize: 9 }}>{club.tier} · {snapshot ? `${snapshot.activeMemberCount} active` : "Waiting for data"}</div></td>
                    {!snapshot || !forecast ? <td colSpan="7" style={{ padding: "11px 12px", color: "#ae9aa5", fontSize: 11 }}>No current club data yet. Analysis will appear when its daily feed exists.</td> : <>
                      <td style={{ padding: "11px 12px", color: "#f3e9ee", fontWeight: 700, fontSize: 11 }}>{fmt(forecast.currentGain)}</td>
                      <td style={{ padding: "11px 12px", fontSize: 11 }}><div style={{ color: "#ffd0d8", fontWeight: 700 }}>{fmt(snapshot.clubTarget)}</div><div style={{ color: "#a18c98", fontSize: 9 }}>{fmt(snapshot.perMemberTarget)} × {snapshot.activeMemberCount}</div></td>
                      <td style={{ padding: "11px 12px", color: "#f3e9ee", fontWeight: 800, fontSize: 12 }}>{fmt(forecast.forecast)}</td>
                      ...
                      <td style={{ padding: "11px 12px", color: "#f3e9ee", fontSize: 11, fontWeight: 700 }}>{official?.ready ? <><div>≈#{official.projectedRank.toLocaleString()}</div><div style={{ color: "#60a5fa", fontSize: 9 }}>{official.projectedTier} · thresholds + history</div></> : <div style={{ color: "#ae9aa5", fontSize: 9 }}>{official?.status === "collecting" ? "Collecting thresholds" : "Not ready"}</div>}</td>
                      <td style={{ padding: "11px 12px" }}><Badge color={forecast.confidenceColor}>{forecast.confidence}</Badge><div style={{ color: "#a18c98", fontSize: 9, marginTop: 4 }}>{forecast.historicalSamples} similar prior months</div></td>
                    </>}
                  </tr>
                );
              })}
    ...
    return (
      <div style={{ display: "grid", gridTemplateColumns: "minmax(190px, 260px) minmax(260px, 1fr)", gap: 22, alignItems: "center" }} className="insights-donut-grid">
        <div style={{ display: "flex", justifyContent: "center" }}>
          <div title="Player contribution share" style={{ width: 220, aspectRatio: "1", borderRadius: "50%", background: `conic-gradient(${stops.join(",")})`, position: "relative", boxShadow: "0 0 30px rgba(169,63,86,0.12)" }}>
            <div style={{ position: "absolute", inset: "27%", borderRadius: "50%", background: "#1d171c", border: "1px solid #50333f", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center", padding: 8 }}><div style={{ color: "#f3e9ee", fontWeight: 800, fontSize: 16 }}>{fmt(resilience.totalGain)}</div><div style={{ color: "#ae9aa5", fontSize: 9 }}>club gain</div></div>
          </div>
        </div>
        <div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {displayRows.map((row, index) => <div key={`${row.name}-${index}`} title={`${row.name}: ${Math.round(row.gain).toLocaleString()} fans (${pct(row.share)})`} style={{ display: "grid", gridTemplateColumns: "10px minmax(120px,1fr) auto auto", gap: 8, alignItems: "center", fontSize: 10 }}><span style={{ width: 8, height: 8, borderRadius: 999, background: COLORS[index % COLORS.length] }} /><span style={{ color: "#ddc9d3", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{row.name}</span><span style={{ color: "#9ca3af", fontWeight: 700 }}>{fmt(row.gain)}</span><span style={{ color: "#f3e9ee", fontWeight: 800, minWidth: 48, textAlign: "right" }}>{pct(row.share)}</span></div>)}
          </div>
          {resilience.contributions.length > 10 && <button onClick={() => setExpanded(!expanded)} style={{ marginTop: 12, background: "#261c23", border: "1px solid #50333f", color: "#ffd0d8", borderRadius: 8, padding: "6px 9px", fontSize: 10, fontWeight: 800, cursor: "pointer" }}>{expanded ? "Show top 10 + grouped remainder" : `Show all ${resilience.contributions.length} contributors`}</button>}
        </div>
      </div>
    );
  }
  ...
  function TransferHelper({ transferData, snapshots, analysisDay }) {
    ...
    return <div>
      <ExplanationBox headline="A decision-support list—not an automatic transfer list" meaning={headline} action={actionSummary} color="#f4a4b1" />
      ...
      <div style={{ ...PANEL, marginBottom: 14, borderColor: "#2dd4bf44", background: "rgba(13,45,48,0.24)" }}>
        ...
        <div style={{ color: "#9ca3af", fontSize: 10, lineHeight: 1.6 }}>Excluded automatically: <b style={{ color: "#ddc9d3" }}>{eligibility.excludedInactive || 0}</b> profiles no longer on that club’s current roster, <b style={{ color: "#ddc9d3" }}>{eligibility.excludedNoCurrentMonthHistory || 0}</b> current-roster profiles with no history for that club this month, and <b style={{ color: "#ddc9d3" }}>{eligibility.excludedPartialMonth || 0}</b> late or partial-month records. This prevents end-of-month transfers from being mistaken for poor performance.</div>
      </div>
      <div style={{ ...PANEL, marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "flex-end", marginBottom: 14 }}>
          <div><div style={{ color: "#f3e9ee", fontSize: 15, fontWeight: 900 }}>Member Transfer Helper</div><div style={MUTED}>Uses projected individual pace, the quota for the member’s current tier, recent direction, idle days, and up to three prior archived months. Analysis is through Day {analysisDay}.</div><div style={{ color: "#c3a7b2", fontSize: 9, lineHeight: 1.55, marginTop: 5 }}>Promotion review = projected to reach at least 125% of the current quota and clear the next tier’s quota by 5%, with no falling recent trend. Keep = projected to reach the current quota but no promotion signal. Monitor is only used below 100% of the current quota.</div></div>
      ...
      {!visible.length ? <EmptyState title="No members match this filter" detail="Try another recommendation category or club. A blank result means no member crossed that review threshold." /> : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 10 }} className="insights-transfer-grid">{visible.map((item) => {
        ...
        return <div key={`${item.clubId}-${item.viewerId}`} style={{ ...PANEL, borderColor: `${meta.color}44`, padding: "14px 15px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "flex-start", marginBottom: 11 }}><div><div style={{ color: "#f3e9ee", fontSize: 13, fontWeight: 900 }}>{item.name}</div><div style={{ color: "#ae9aa5", fontSize: 9, marginTop: 2 }}>{item.clubName} · current tier {item.currentTier}</div></div><Badge color={meta.color}>{meta.icon} {meta.label}</Badge></div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 7, marginBottom: 11 }}>
            <div><div style={{ color: "#a18c98", fontSize: 8, textTransform: "uppercase", fontWeight: 800 }}>Projected finish</div><div style={{ color: meta.color, fontSize: 12, fontWeight: 900 }}>{fmt(item.projected)}</div><div style={{ color: "#ae9aa5", fontSize: 8 }}>{pct(item.projectedRatio, 0)} of quota</div></div>
            <div><div style={{ color: "#a18c98", fontSize: 8, textTransform: "uppercase", fontWeight: 800 }}>Recent direction</div><div style={{ color: item.trend === "rising" ? "#34d399" : item.trend === "falling" ? "#f87171" : "#ddc9d3", fontSize: 12, fontWeight: 900, textTransform: "capitalize" }}>{item.trend}</div><div style={{ color: "#ae9aa5", fontSize: 8 }}>{item.idleDays} recent idle day{item.idleDays === 1 ? "" : "s"}</div></div>
            <div><div style={{ color: "#a18c98", fontSize: 8, textTransform: "uppercase", fontWeight: 800 }}>Suggested placement</div><div style={{ color: "#ffd0d8", fontSize: 12, fontWeight: 900 }}>{suggestion}</div><div style={{ color: "#ae9aa5", fontSize: 8 }}>{item.confidence}</div></div>
          </div>
          <div style={{ background: "#151115", borderRadius: 9, padding: "9px 10px", marginBottom: 8 }}><div style={{ color: "#c3a7b2", fontSize: 8, fontWeight: 900, textTransform: "uppercase", marginBottom: 3 }}>Why it was flagged</div><div style={{ color: "#b9b5cf", fontSize: 10, lineHeight: 1.5 }}>{item.commentary}</div></div>
      ...
      {filtered.length > visibleCount && <div style={{ display: "flex", justifyContent: "center", marginTop: 12 }}><button onClick={() => setVisibleCount((count) => count + 12)} style={{ background: "#261c23", border: "1px solid #a93f5666", color: "#ffd0d8", borderRadius: 9, padding: "8px 12px", fontSize: 10, fontWeight: 900, cursor: "pointer" }}>Show {Math.min(12, filtered.length - visibleCount)} more</button></div>}
  ...
  function QuotaStressLab({ snapshot, forecast, stress, snapshots, selectedClubId, setSelectedClubId, analysisDay }) {
    ...
    return <div>
      ...
      <div style={{ ...PANEL, marginBottom: 14 }}>
        ...
        <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", minWidth: 760 }}><thead><tr>{["Scenario", "Stressed finish", "% of quota", "Goal margin", "Recovery needed/day"].map((header) => <th key={header} style={{ color: "#ae9aa5", textAlign: "left", fontSize: 9, textTransform: "uppercase", letterSpacing: "0.08em", padding: "10px 9px", borderBottom: "1px solid #3b2932" }}>{header}</th>)}</tr></thead><tbody>{stress.scenarios.map((scenario) => <tr key={scenario.key}><td style={{ padding: "11px 9px", borderBottom: "1px solid #3b2932" }}><div style={{ color: "#f3e9ee", fontSize: 11, fontWeight: 800 }}>{scenario.label}</div><div style={{ ...MUTED, fontSize: 9, marginTop: 2 }}>{scenario.description}</div></td><td style={{ padding: "11px 9px", borderBottom: "1px solid #3b2932", color: "#ffd0d8", fontWeight: 800 }}>{fmt(scenario.stressedForecast)}</td><td style={{ padding: "11px 9px", borderBottom: "1px solid #3b2932", color: scenario.gap >= 0 ? "#34d399" : "#f87171", fontWeight: 900 }}>{pct(scenario.targetRatio)}</td><td style={{ padding: "11px 9px", borderBottom: "1px solid #3b2932", color: scenario.gap >= 0 ? "#34d399" : "#f87171", fontWeight: 800 }}>{scenario.gap >= 0 ? `+${fmt(scenario.gap)}` : `-${fmt(Math.abs(scenario.gap))}`}</td><td style={{ padding: "11px 9px", borderBottom: "1px solid #3b2932", color: scenario.dailyRecoveryNeeded ? "#fbbf24" : "#ae9aa5", fontWeight: 800 }}>{scenario.dailyRecoveryNeeded ? fmt(scenario.dailyRecoveryNeeded) : "None"}</td></tr>)}</tbody></table></div>
      </div>
      <div style={PANEL}><div style={{ color: "#f3e9ee", fontSize: 13, fontWeight: 900, marginBottom: 10 }}>Largest remaining dependencies</div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(210px,1fr))", gap: 9 }}>{stress.topThree.map((member, index) => <div key={member.viewerId} style={{ background: "#151115", border: "1px solid #3b2932", borderRadius: 10, padding: "11px 12px" }}><div style={{ color: index === 0 ? "#fbbf24" : "#f3e9ee", fontWeight: 900, fontSize: 12 }}>#{index + 1} {member.name}</div><div style={{ color: "#c3a7b2", fontSize: 10, marginTop: 4 }}>{fmt(member.projectedRemaining)} expected remaining · {pct(stress.totalProjectedRemaining > 0 ? member.projectedRemaining / stress.totalProjectedRemaining : 0)} of club’s remaining member output</div></div>)}</div></div>
    </div>;
  }
  ...
  function DeeperInsightsPage({ clubs = [], clubData = {}, archiveManifest = null, rankThresholdData = null, today = 1, dim = 31, monthKey = "", archiveMonth = "", isArchiveView = false }) {
    ...
    return (
      <div>
        <div style={{ ...PANEL, marginBottom: 14, background: "linear-gradient(135deg, rgba(169,63,86,0.16), rgba(29,23,28,0.94) 48%, rgba(37,99,235,0.10))" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start", flexWrap: "wrap" }}>
            <div style={{ maxWidth: 760 }}><div style={{ color: "#fff1f5", fontSize: 25, fontWeight: 900 }}>Deeper Insights</div><div style={{ color: "#c3a7b2", fontSize: 12, lineHeight: 1.65, marginTop: 5 }}>Six practical analysis views cover forecasts, momentum, historical twins, member patterns, transfer reviews, and club stress tests. Every view includes a plain-language interpretation.</div></div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "flex-end" }}><Badge color="#f4a4b1">{monthLabel(monthKey || snapshots[0]?.monthKey)}</Badge><Badge color={isArchiveView ? "#ffd0d8" : "#34d399"}>{isArchiveView ? "Past-month view" : "Current-month data"}</Badge><Badge color={historyStatus.loading ? "#fbbf24" : "#60a5fa"}>{historyStatus.loading ? `Loading prior months ${historyStatus.loaded + historyStatus.failed}/${historyStatus.total}` : `${historyStatus.loaded} prior months loaded`}</Badge></div>
          </div>
          <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "minmax(230px,1fr) auto", gap: 14, alignItems: "end" }} className="insights-day-control">
            <div><div style={{ display: "flex", justifyContent: "space-between", gap: 8, marginBottom: 7 }}><span style={{ color: "#ddc9d3", fontSize: 11, fontWeight: 800 }}>Analyze as of Day {analysisDay}</span><span style={{ color: "#ae9aa5", fontSize: 10 }}>Latest available: Day {latestDay}</span></div><input aria-label="Analysis day" type="range" min="1" max={latestDay} value={analysisDay} onChange={(event) => setAnalysisDay(Number(event.target.value))} style={{ width: "100%", accentColor: "#a93f56" }} /></div>
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>{sliderStops.map((day) => <button key={day} onClick={() => setAnalysisDay(day)} style={{ background: analysisDay === day ? "#a93f56" : "#261c23", border: `1px solid ${analysisDay === day ? "#c35b70" : "#50333f"}`, color: "#f3e9ee", borderRadius: 8, padding: "6px 8px", fontSize: 9, fontWeight: 800, cursor: "pointer" }}>Day {day}</button>)}</div>
          </div>
        ...
        <div style={{ display: "flex", gap: 7, flexWrap: "wrap", marginBottom: 14 }}>{sections.map(([key, label]) => <button key={key} onClick={() => setActiveSection(key)} style={{ background: activeSection === key ? "#a93f56" : "#1d171c", border: `1px solid ${activeSection === key ? "#c35b70" : "#3b2932"}`, color: activeSection === key ? "#fff" : "#9ca3af", borderRadius: 9, padding: "8px 11px", fontSize: 10, fontWeight: 800, cursor: "pointer" }}>{label}</button>)}</div>
        ...
        {activeSection === "forecast" && <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(175px,1fr))", gap: 10, marginBottom: 14 }}>
            ...
            <MetricCard label="Best network estimate" value={fmt(overall.forecast)} sub={`${fmt(overall.low)} – ${fmt(overall.high)} working range`} color="#ffd0d8" />
